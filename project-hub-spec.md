...
Workspace: `techgurusit`. Auth: HTTP Basic с **Atlassian account email + API token** (создаётся в `id.atlassian.com` → Security → **Create API token with scopes** → Bitbucket → `read:repository:bitbucket`).

⚠️ **App passwords задепрекейчены Atlassian с сентября 2025** — новые создать нельзя, только API tokens. Схема Basic Auth ровно та же (`email:token`), код менять не пришлось — поменялись только UI-лейблы и инструкция выпуска.
...
Секции:
- **Bitbucket** — workspace, email (Atlassian account), API token (input type=password), кнопка Test connection
...
- Bitbucket Cloud workspace `techgurusit`, доступ через **API token** (Atlassian email + token, Basic Auth). App passwords задепрекейчены с сен. 2025 — новые не создать.
