...
`id.atlassian.com → Security → API tokens → Create API token` (классический,
не "with scopes").
...
Отдельный классический токен (Atlassian не даёт один токен на оба
продукта). Создаётся той же кнопкой "Create API token" на той же странице.
...
> **Не используй "Create API token with scopes"** — у scoped-токенов есть
> известный баг Atlassian: JQL `currentUser()` не резолвится через Bearer-auth,
> и My Tasks показывает пустоту. Классические токены работают через Basic
> auth и резолвят `currentUser()` корректно.
...
- Если scoped-токен на Bitbucket: scopes должны быть как минимум
  `read:account`, `read:workspace:bitbucket`, `read:repository:bitbucket`,
  `read:pipeline:bitbucket`. Лучше — классический токен.
...
Токен скорее всего без `read:pipeline:bitbucket` scope. Если ты
используешь scoped-токен — пересоздай со scope. Если классический —
у него уже есть.
